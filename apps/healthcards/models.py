...
from ..healthcards_python_sample_scripts.generate_random_jwks import generate_signing_key, generate_encryption_key, generate_keyset
from ..healthcards_python_sample_scripts.utils import (encode_to_numeric, create_qr_code, 
                SMART_HEALTH_CARD_PREFIX, sample_payload, encode_vc)
import qrcode
...
class SmartHealthJWKS(models.Model):
    ...
    kid = models.CharField(max_length=255, default='', blank=True)
...
class SmartHealthCard(models.Model):
    ...
    def save(self, commit=True, *args,  **kwargs):
        ...
        if self.payload and not self.shc_qrcode:
            ...
            numeric_encoded_payload = encode_to_numeric(encode_vc(json.loads(self.payload), private_keys['keys'][0], shc_jwks.kid))
